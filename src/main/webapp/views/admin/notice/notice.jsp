...
<head>
    ...
    <script>
        function searchby() {
            ...
            $.ajax({
                url:'<c:url value="/admin/notice/searchby"/>',
                method:'post',
                data:{'op':op, 'txt':txt},
        ...
        function sortby() {
            ...
            $.ajax({
                url:'<c:url value="/admin/notice/sortby"/>',
                method:'post',
                data:{'op':op},
                success: res => success(res)
            });
        ...
        function listup() {
            ...
            $.ajax({
                url:'<c:url value="/admin/notice/listup"/>',
                method:'post',
                data:{'op':op},
                success: res => success(res)
            });
        ...
        function success(res) {
            let html = '';
            html += `<tr> <th>글번호</th> <th>제목</th> <th>작성자</th> <th>작성일</th> </tr>`
            res.forEach(function(n) {
                html += '<tr>' +
                    '<td>' + n.noticeIdx + '</td>' +
                    '<td><a href="/admin/notice/detail?noticeidx=' + n.noticeIdx + '">' + n.noticeTitle + '</a></td>' +
    ...
    <div id="adminSection">
        ...
        <div class="adminDiv">
            ...
            <select class="size" id="sort_box" onchange="sortby()">
                ...
                <option value="2">등록일 오름차순</option>
                <option value="3">등록일 내림차순</option>
            </select>
        </div>
        ...
        <div class="">
            <table class="adminTable" id="results">
                <thead>
                <tr>
                    <th>글번호</th>
                    <th>제목</th>
                ...
                <c:forEach var="n" items="${notices}">
                    <tr>
                        <td>${n.noticeIdx}</td>
                        <td>
                            <a href="<c:url value="/admin/notice/edit"/>?noticeidx=${n.noticeIdx}">${n.noticeTitle}</a>
                        </td>
                        <td>${n.noticeMemberId}</td>
                        <td>
        ...
        <br/>
        <div id="btn_write">
            <img onclick="window.location.href='<c:url value="/admin/notice/write"/>'"
